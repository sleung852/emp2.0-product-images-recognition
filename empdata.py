...
class EmpImageData:
    ...
    def create_dataset(self, label_col, file_col, folder_dir, min_n=100, train_val_ratio=0.8):

        df=self.data[[label_col,file_col]]
        df_sum = df.groupby('LABEL').count().sort_values('PRODUCT_ID', ascending=False)
        keepindex=df_sum[df_sum['PRODUCT_ID']>=min_n].index
        #print(keepindex)
        df = df[df['LABEL'].apply(lambda x: True if x in keepindex else False)]
    ...
    def mktorchdataloaders(self, transformations=None, batch_size=4):
        keys=['train','val']
        product_datasets = {key: ImageData(self.df_data[key], transforms=transformations) for key in keys}
